...
import { z } from "zod";
import { createUserAccount, fetchAllUserComments, fetchAllUserCommunities, fetchAllUserFollowed, fetchAllUserFollowedCount, fetchAllUserFollowing, fetchAllUserFollowingCount, fetchAllUserLikedPosts, fetchAllUserPosts, fetchAllUserSaved, fetchUserAccount, fetchUserAccountById } from "~/lib/data";
...
export const userRouter = createTRPCRouter({
    ...
    .query(async ({ input }) => {
        ...
        const { allPosts, nextCursor, hasNextPage } = await fetchAllUserFollowed( cursor, limit, userId );
    ...
    .query(async ({ input }) => {
        ...
        const { allPosts, nextCursor, hasNextPage } = await fetchAllUserFollowing( cursor, limit, userId );
    ...
    .query(async ({ input }) => {
        ...
        const { allCommunities, nextCursor, hasNextPage } = await fetchAllUserCommunities( cursor, limit, userId );
    ...
    .query(async ({ input }) => {
        ...
        const { allPosts, nextCursor, hasNextPage } = await fetchAllUserSaved( cursor, limit, userId );
    ...
    .query(async ({ input }) => {
        ...
        const { allPosts, nextCursor, hasNextPage } = await fetchAllUserPosts( cursor, limit, userId );
    ...
    .query(async ({ input }) => {
        ...
        const { allPosts, nextCursor, hasNextPage } = await fetchAllUserComments( cursor, limit, userId );
    ...
    .query(async ({ input }) => {
        ...
        const { allPosts, nextCursor, hasNextPage } = await fetchAllUserLikedPosts( cursor, limit, userId );
    ...
    .mutation(async ({input, ctx}) => {
        ...
        console.log("Edit method called with:", { url_icon, url_banner, description });
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      try {
        ...
        if (existingUser && existingUser.length > 0) {
            ...
            throw new TRPCError({ code: "CONFLICT", message: "USER MAIL ALREADY EXISTS" });
        }
