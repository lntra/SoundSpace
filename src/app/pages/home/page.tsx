...
export default function HomePage() {
    ...
    const welcomeNotification : Notifications = {
        content: "Welcome to SoundSpace! Check our communities to get to know our space!",
        seen: false, 
        img: `${LogoHD.src}`,
        created_at: new Date(), 
    }
    ...
    useEffect(() => {
        ...
        if(checkIfNotification === null || checkIfNotification == "[]"){
            ...
            localStorage.setItem('userNotifications', JSON.stringify(notificationsSet));
        } 
    },[])

    return <>
        <main className={`${dark ? "bg-gray-900 text-white"  : "bg-sp-greyish text-white"}`}>
            <div className={`font-['Lato'] ${dark ? "bg-gray-900 text-white"  : "bg-sp-greyish text-white"}`}>
