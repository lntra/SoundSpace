...
const BarDotsBlack = ( {options, user_id ,post_id, isAuthor, handleClickSelect, setSavedUsers} : BarDotsBlackProps ) => {
    ...
    return <>
        <div className="relative flex">
            <button className="w-full" onClick={handleClick} ref={dropdownState}>
                <svg width="17" height="17" viewBox="0 0 17 1" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={`dropdown  ${dots ? 'dropdown-enter' : 'dropdown-exit'}`}>

            {dots && (
                    <div
                        ref={dropdownMenuRef}
                        className={`absolute mt-2 left-[-15px] w-48 rounded-full flex shadow-lg text-black `}
                    >
                        <PlaceholderProps className="flex w-full">
                            <ul className="text-lg bg-white rounded-lg w-full self-start">
                                {options.map((e, index) => (
                                    <li
                                        key={index}
                                            ...
                                            {!isAuthor && index === 0 && (
                                                    <>
                                                        <svg width="32" height="32" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M12 4C7.6 4 4 7.6 4 12C4 16.4 7.6 20 12 20C16.4 20 20 16.4 20 12C20 7.6 16.4 4 12 4ZM12 6C13.3 6 14.5 6.4 15.5 7.1L7.1 15.5C6.4 14.5 6 13.3 6 12C6 8.7 8.7 6 12 6ZM12 18C10.7 18 9.5 17.6 8.5 16.9L16.9 8.5C17.6 9.5 18 10.7 18 12C18 15.3 15.3 18 12 18Z" fill="#53337B"/>
                                                        </svg>
                                                    </>
                                            )}
                                            {!!isAuthor && index === 0 && (
                                                    <>
                                                        <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M19.2 7.2L17 5L12.1 10L7.2 5L5 7.2L10 12.1L5 17L7.2 19.2L12.1 14.2L17 19.2L19.2 17L14.2 12.1L19.2 7.2Z" fill="#53337B"/>
                                                        </svg>
                                                    </>
                                            )}
                                            {index === 1 && (
                                                <>
                                                    <svg width="32" height="32" viewBox="0 2 36 35" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M15 6.25V10.75H19.5L15 6.25Z" fill={`#53337B`}/>
                                                        <path d="M19.5 12.25H13.5V6.25H6V24.25H19.5V12.25Z" fill={`#53337B`}/>
                                                        <path d="M25.5 12.25V16.75H30L25.5 12.25Z" fill={`#53337B`}/>
                                                        <path d="M24 12.25H21V25.75H16.5V30.25H30V18.25H24V12.25Z" fill={`#53337B`}/>
                                                    </svg>
                                                </>
                                            )}
                                            {index === 2 && (
                                                <>
                                                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M14 7H8.9C8.9 7 4 7 4 12C4 15.9 7 20 7 20C7 20 5.3 13 8.8 13H14V16L20 10L14 4V7Z" fill="#53337B"/>
                                                    </svg>
                                                </>
                                            )}
                                            <p className="text-center py-2 px-1">{e}</p>
                                        </div>
                                    </li>
                                ))}
                            </ul>
            ...
            <div className="fixed inset-0 top-0 left-0 h-screen w-screen flex items-center justify-center bg-black bg-opacity-60 z-50 transition-opacity duration-1000 ease-in-out">
                <div className="bg-white p-6 rounded-lg w-80 relative opacity-100 transition-opacity duration-1000 ease-in-out">
                    <button
                    onClick={handlePopUp}
