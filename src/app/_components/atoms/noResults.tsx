...
const NoResults = ( {dark , text} : NoResultsProps ) => {
    return <>
        <div className={`${!dark === true ? "opacity-70" : "opacity-70"} flex flex-col h-full justify-center items-center align-middle  border-sp-purpleBright2 rounded-3xl`}>
            <div className="w-48"> 
                ...
                fillRule="evenodd" 
                clipRule="evenodd" 
                fill={`${!dark === true ? "#6232DA" : "#A9FB1A"}`}
                viewBox="0 0 512 512.01">
                <path fillRule="inherit" 
            ...
            <h1 className={`font-bold text-2xl p-4 ${!dark === true ? "text-sp-purpleBright2" : "text-sp-accent"}`}>{text ? text : "Sorry, we couldn't find any results"}</h1>
        </div>
    </>
}
