...
import { Communities, Posts as PostsType } from "~/lib/definitions"
...
const Posts : React.FC<PostsProps> = ( {forYouRoute, search, setSearch, id, tags, tagsState, dark} ) => {
    ...
    useEffect(() => {
        if (data) {
            ...
            if(id !== undefined) {
                let mergedPosts = [...userPosts.filter((posts: PostsType) => posts.community_id === id), ...serverPosts];

                mergedPosts = Array.from(new Map(mergedPosts.map(post => [post.id, post])).values());

                mergedPosts = mergedPosts.filter((post: PostsType) => post.id !== "d1adf5ec-c0af-4608-8aa2-72c0f9228312");
                ...
                switch(search){
                    case "Trending" :
                        ...
                        const recentPosts = mergedPosts.filter((post: PostsType) => new Date(post.created_at) >= fourteenDaysAgo);
                        const olderPosts = mergedPosts.filter((post: PostsType) => new Date(post.created_at) < fourteenDaysAgo);

                        recentPosts.sort((a: PostsType, b: PostsType) => (b.likes || 0) - (a.likes || 0));
                        olderPosts.sort((a: PostsType, b: PostsType) => (b.likes || 0) - (a.likes || 0));
                    ...
                    break;
                    case "Hot Topics" :
                        mergedPosts.sort((a: PostsType, b: PostsType) => (b.comments || 0) - ((a.comments || 0)));
                        setuserPosts(mergedPosts)
                    break;
                    case "Recent" :
                        console.log(mergedPosts)
                        mergedPosts.sort((a: PostsType, b: PostsType) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
                        setuserPosts(mergedPosts)
                    break;
                    case "Top Rated" :
                        mergedPosts.sort((a: PostsType, b: PostsType) => (b.likes || 0) - ((a.likes || 0)));
                        setuserPosts(mergedPosts)
                    break;
                }
                ...
                if(blocked.length > 0) {
                    ...
                    mergedPosts = mergedPosts.filter((post : PostsType) => !blockedUserIds.includes(post.user_id));
                ...
                if(forYouRoute) {
                    ...
                    const followedCommunities = localFollowing.map((community: Communities) => community.id)

                    console.log(followedCommunities)

                    mergedPosts = mergedPosts.filter((post: PostsType) => followedCommunities.includes(post.community_id));    
    ...
    const handleDelete = ( tagText : string ) => {
        ...
        const existeSaved = localSaved.filter((post : PostsType) => post.id !== tagText);
        localStorage.setItem("userPosts", JSON.stringify(existeSaved));

        const localLiked = JSON.parse(localStorage.getItem("likedPosts") || "[]");
        const existeLikes = localLiked.filter((post : PostsType) => post.id !== tagText);
        ...
        const existeComments = localComments.filter((post : PostsType) => post.id !== tagText);
