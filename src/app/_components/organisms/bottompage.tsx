...
const BottomPage = ( { disabledIds, dark } : NewsAsProps ) => {
    ...
    return <>
        <div className={`${dark ? "bg-gray-900 text-white" : "bg-white text-black"} overflow-x-hidden`}>
            <div className="flex flex-wrap justify-center items-center m-6">
                <Trending dark={dark} search={search} setSearch={setSearch} topics={["Trending","Articles","News","Recent"]}></Trending>
            </div>
            <div className={`grid grid-cols-12 gap-2 grid-rows-auto w-full overflow-x-hidden`}>
                {data && (
                    <BottomNews dark={dark} disabledIds={disabledIds} news={data}></BottomNews>
                )}
                {isLoading && (
                    <div className="col-start-2 col-end-9 col-span-9 row-start-1">
                        <LoadingPage></LoadingPage>
                    </div>   
                )}
                <div className={`hidden p-3 lg:block lg:col-span-4 xl:col-span-3 lg:row-start-1 lg:row-span-3 h-[350px] ${dark ? "text-white bg-gray-900" : "text-black bg-white"}`}>
                    <PlaceholderProps dark={dark} className={` ${dark ? "text-white bg-gray-950" : "text-black bg-white"}`} title="Trending Communities">
                        <div className="p-3  flex flex-col">
                            <Link href={`/pages/community/${"97f4d418-eb4a-425d-97d0-83f6acbc435a"}`}>
                                <div className={` ${dark ? "hover:bg-gray-800" : "hover:bg-gray-200"} flex items-center align-middle border-b-[1px] py-2 border-sp-purpleBright2`}>
                                        ...
                                        textStyle="text-base" style="w-8 h-8 mr-2 rounded-full" text="The Band, from Outer Space" userId={'97f4d418-eb4a-425d-97d0-83f6acbc435a'}></UserPost>          
