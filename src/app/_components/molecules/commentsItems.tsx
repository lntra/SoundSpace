...
const CommentsItems: React.FC<CommentsItemsProps> = ({
  userComments,
  setUserComments,
  likedComments,
  setLikedCommments,
  ...
  const handleUnlikedComments = () => {
    if (comment && comment.id) {
      if (isLiked) {
        setLikedCommments((e) =>
          e.filter((likedPost) => likedPost !== comment.id),
        );
        setIsLiked(false);
      }
    }
          ...
          <div className="flex w-full flex-col items-end justify-end">
            {comment.replies.map((comment, index) => (
              <div
                ...
                className={`${iteration < 5 ? "w-[90%]" : "w-[100%]"}`}
              >
                <CommentsItems
                  userComments={userComments}
                  setUserComments={setUserComments}
                  ...
                  setLikedCommments={setLikedCommments}
